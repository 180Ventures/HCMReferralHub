...
const ResetPasswordForm: FC<IProps> = ({}) => {
  const { inititalValues, showPassword, onSubmitForm, onToggleShowPassword, onGotoSignUp } = useResetPasswordHooks();
  return (
    <div className="flex flex-col justify-center items-center lg:max-w-sm w-full h-full">
      <div className="w-[90%] lg:w-auto flex flex-col justify-center items-center">
        <div className="d-flex flex-col items-start w-full md:w-[370px]">
          <p className={`text-2xl text-left font-medium text-gray20`}>New Password</p>
          <p className="text-sm font-normal mt-2">Please create a new password that you don’t use on any other site.</p>
        </div>
        <Formik
        ...
        >
          {(props) => (
            <form onSubmit={props.handleSubmit} className="mt-2 2xl:mt-6 w-full md:w-[370px]">
              ...
              <button type="submit" className="mt-6 w-full bg-orangeLight rounded-lg h-12">
                <p className="text-base font-medium text-white font-Poppins">Send Recovery Email</p>
              </button>
              <p className="text-sm font-normal text-gray20 text-center mt-4 cursor-pointer">
                Just remember?{' '}
                <span onClick={onGotoSignUp} className="text-orangeLight font-semibold cursor-pointer">
                  Sign Up
                </span>
              </p>
            </form>
